...
let sendstate x =
    browser.runtime.sendMessage (box (x |> StateUpdate)) |> Promise.start; x
...
let LoopArr arr total dowhat = async {
    let timesep = 5
    arr |> Array.iteri (fun i x -> window.setTimeout((fun () -> dowhat i x), i*timesep) |> ignore)
...
let GetHistory hdays maxres = async {
    let endtime = ((DateTimeOffset.Now.ToUnixTimeSeconds() |> float)-(864000.0*hdays))*1000.0
    let options =
        createObj [
            "text" ==> "";
            "startTime" ==> 0.0;
...
let HandleMessage x =
    match x with
        ...
        | GetStorage ->
            BrowserLocalStorage.load "tomeStorage" |> UpdateStorage |> box |> browser.runtime.sendMessage |> Promise.start
        | _ -> ()
